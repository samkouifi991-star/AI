...
type Doc = { id: string; file_name: string; doc_type: string | null; status: string; error_message?: string | null; created_at: string };
...
export default function TeachPage() {
  ...
  const [websiteImportMsg, setWebsiteImportMsg] = useState<string | null>(null);

  async function refresh(bizId: string, isRestaurant: boolean) {
    const [{ data: open }, { data: resolved }, { data: allDocs, count: docCount }, { count: chunks }, { data: business }, { count: itemCount }, { count: hoursCount }] = await Promise.all([
      supabase
        .from('knowledge_gaps')
      ...
      supabase
        ...
        .select('id, file_name, doc_type, status, error_message, created_at', { count: 'exact' })
        .eq('business_id', bizId)
  ...
  async function importWebsite() {
    if (!websiteUrl.trim() || !businessId) return;
    setWebsiteImporting(true);
    setWebsiteImportMsg(null);
    ...
    if (!res.ok) {
      setWebsiteImportMsg(data.error ?? 'Could not import that website.');
      return;
    }
    setWebsiteImportMsg(`Read ${data.pagesFetched} page${data.pagesFetched === 1 ? '' : 's'}.`);
    await refresh(businessId, businessType === 'restaurant');
  }
    ...
    <div style={{ maxWidth: 1080 }} className="font-instrument space-y-3">
      ...
      <div className="bg-bp-surface border border-bp-border" style={{ borderRadius: 14 }}>
        <div className="border-b border-bp-border-soft" style={{ padding: '15px 18px 13px' }}>
          <h2 className="font-grotesk" style={{ fontSize: 15, fontWeight: 600, letterSpacing: '-0.01em' }}>What {aiName} knows</h2>
          <p className="text-bp-ink-muted" style={{ fontSize: 12.5, marginTop: 2 }}>
            {chunkCount} piece{chunkCount === 1 ? '' : 's'} of your business she can search during a call.
          </p>
        </div>
        {knowledgeRows.map((r, i) => (
          <div key={r.label} className={i > 0 ? 'border-t border-bp-border-faint' : ''} style={{ padding: '16px 22px', display: 'flex', alignItems: 'flex-start', gap: 12 }}>
            <span className={r.ok ? 'bg-bp-good' : 'bg-bp-ink-faint'} style={{ width: 7, height: 7, borderRadius: '50%', flex: '0 0 7px', marginTop: 5 }} />
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, fontWeight: 500 }}>{r.label}</div>
              <div className="text-bp-ink-muted" style={{ fontSize: 13, lineHeight: 1.55, marginTop: 2 }}>{r.state}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {problems.length > 0 && (
        <div className="bg-bp-surface border border-bp-border" style={{ borderRadius: 14, padding: '20px 22px' }}>
          <h2 className="font-grotesk" style={{ fontSize: 15, fontWeight: 600, letterSpacing: '-0.01em' }}>Gaps and problems</h2>
          <p className="text-bp-ink-muted" style={{ fontSize: 12.5, marginTop: 2, marginBottom: 10 }}>Only things we can actually see in your data.</p>
          <ul style={{ display: 'flex', flexDirection: 'column', gap: 8, paddingLeft: 18 }}>
            {problems.map((p) => (
              <li key={p} className="text-bp-warn-ink" style={{ fontSize: 13.5, lineHeight: 1.55 }}>{p}</li>
            ))}
          </ul>
        </div>
      )}
      ...
      <div className="bg-bp-surface border border-bp-border" style={{ borderRadius: 14, padding: '20px 22px' }}>
        <h2 className="font-grotesk" style={{ fontSize: 15, fontWeight: 600, letterSpacing: '-0.01em', marginBottom: 12 }}>Add something she should know</h2>
        <label
        ...
        <div style={{ marginTop: 14, display: 'flex', gap: 8 }}>
          ...
          <button className="btn-secondary shrink-0" onClick={importWebsite} disabled={websiteImporting || !websiteUrl.trim()}>
            {websiteImporting ? 'Reading…' : 'Import from website'}
          </button>
        </div>
        {websiteImportMsg && <div className="text-bp-ink-muted" style={{ fontSize: 12.5, marginTop: 6 }}>{websiteImportMsg}</div>}
      </div>
