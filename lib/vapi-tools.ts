...
export const VAPI_TOOLS = [
  ...
  },
  {
    ...
    description:
      "Check whether the business is open right now, or what its hours are for a given day — computed from the real saved schedule and the current time, not a guess. Always call this for questions like 'are you open right now', 'what time do you close tonight', or 'are you open Sunday' rather than answering from memory.",
    parameters: {
      type: 'object',
      properties: {
        day: {
          type: 'string',
          description: "Optional. A specific day to ask about, e.g. 'Sunday'. Omit to check right now."
        }
      }
    }
  },
...
export function buildSystemPrompt(
  ...
  const toneClause = TONE_CLAUSES[tone ?? 'friendly'] ?? TONE_CLAUSES.friendly;
