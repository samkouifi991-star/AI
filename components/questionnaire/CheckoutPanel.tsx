...
export function CheckoutPanel({
  ...
  translationPackage: { priceCents: number; alreadyPaid?: boolean } | null
}) {
  const [includePrintMail, setIncludePrintMail] = useState(false)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const includeTranslation = translationPackage && !translationPackage.alreadyPaid
  const total = serviceFeeCents + (includePrintMail ? printMailFeeCents : 0) + (includeTranslation ? translationPackage.priceCents : 0)
  const smartUsaVisaTotal = serviceFeeCents + (includeTranslation ? translationPackage.priceCents : 0) + (includePrintMail ? printMailFeeCents : 0)
    ...
    <div className="card">
      ...
      <div className="mt-4 space-y-2 text-sm">
        <div className="flex justify-between">
          <span className="text-ink-600">{applicationLabel}</span>
        ...
        {translationPackage && (
          <div className="flex justify-between">
            <span className="text-ink-600">
              Certified Document Translation Package
              {translationPackage.alreadyPaid && <span className="ml-2 text-xs text-success-600">Paid</span>}
            </span>
            <span className="font-medium text-ink-900">{formatCents(translationPackage.priceCents)}</span>
          </div>
        )}
      ...
      <div className="mt-4 flex justify-between border-t border-ink-100 pt-4 text-sm text-ink-600">
        <span>Smart USA Visa Services</span>
        <span>{formatCents(smartUsaVisaTotal)}</span>
      </div>
      ...
      {governmentFees.length > 0 && (
        <div className="mt-6 rounded-xl bg-warning-50 p-4 text-sm text-ink-700">
          ...
          <p className="mt-2 text-xs">
            ...
            never collects them on the government's behalf, and this amount is not Smart USA Visa
            revenue.
          </p>
        </div>
      )}
